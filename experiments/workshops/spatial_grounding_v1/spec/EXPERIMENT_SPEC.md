...
**Current status:** Scene generation, physical scoring, recording, release construction and the production worker are implemented. Scripted physical qualification is in progress. Complete selected fixtures, relocated runtime assets, native N3/D1 reset/action behavior and forecast time/camera mappings still require their actual qualification receipts. No clean learned-policy results are available and no study launch is authorized. See [current status](../../../../REPOSITORY_STATUS.json).
