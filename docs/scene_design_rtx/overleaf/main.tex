...
\textbf{Camera framing.} The prospective camera revision \texttt{close-oblique-v2-20260924} places two elevated exterior views on the far side of the table, aimed toward the task area and back toward the robot. Positions, orientations and focal lengths are registered for each of the 87 layouts and remain identical across its six wording/goal conditions and both models. The robot, wrist camera, objects, supports, scoring and prompts are unchanged. Figure~10 of the TRI LBM paper~\cite{trilbm}, on PDF page~14, is a qualitative reference for elevated tabletop framing; we do not reproduce its hardware or camera calibration. Camera fit and resulting magnification vary with task geometry, so cross-family visual scale is not a controlled concept effect.
...
All 87 revised scenes and 1,044 cell bindings passed offline destination materialization. Offline image preprocessing passed on six native inputs and one synthetic input, without model requests. These engineering checks do not qualify N3/D1 model inputs, future-frame time maps, or semantic prediction labels. Model requests and learned-policy episodes are both zero; destination and model/runtime checks remain handoff gates, not evidence of model performance.
