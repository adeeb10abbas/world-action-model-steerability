...
\newcommand{\status}{\textbf{Research plan --- SGW-01, version 1.1; clean study, 24 September 2026.} Scene generators are implemented and physical qualification is underway. This document reports a planned study, not model results.}
...
All trials use clean tabletop scenes in DROID/RoboLab. Before selection, each candidate must pass both physical goals across three scripted resets, satisfy reset agreement within 3 mm and 2 degrees, and retain measured object, support, camera and asset identities. Scripted success establishes fixture feasibility only. Finite candidate lists and their seeded selection order are declared before their outcomes; candidates are never selected by policy success. Each family requires 29 distinct qualifying layouts: one pilot on the seeded side, two development layouts per side and 12 confirmation layouts per side. LAT balances the cube's starting approach side relative to the fixed robot frame; HEIGHT balances the upper-support side; DIST balances bowl versus plate side. Side assignments are checked from measured geometry. Both models use the same accepted layouts. The three generators and illustrative qualified examples are available, while qualification of the full 87-layout registry remains incomplete.
...
The current work completes and physically validates clean LAT, HEIGHT and DIST scenes. A portable registry must identify every selected layout's exact design, measured geometry and independent six-trial verification while retaining rejected and incomplete candidates. Generator implementation alone does not establish a qualified layout or a complete cohort.
