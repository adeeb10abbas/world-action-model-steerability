...
| Do these effects differ across spatial tasks? | Separate LAT, HEIGHT and DIST analyses for N3, E3 and F3 | Nine model-family reports, including side strata |
...
| Stage | Layouts per family | Episodes per model-family | Across three checkpoints and three families |
|---|---:|---:|---:|
| Recording pilot | 1 | 6 | 54 |
| Development | 4 | 24 | 216 |
| Confirmation | 24 | 144 | 1,296 |
| Total | 29 | 174 | **1,566** |

There are **261 six-cell blocks**. All three checkpoints receive the same accepted layouts. Pilot and development observations do not enter confirmation estimates. The independent unit is the layout, not a frame, request or repeated condition. Effective seeds are model-specific; repeated instructions do not create new independent layouts.
...
The registered [`close-oblique-v3-full-objects-20260924` camera configuration](../../experiments/workshops/spatial_grounding_v1/close_cameras.json) changes both exterior camera poses and focal lengths for all 87 unique layouts. Two elevated oblique cameras on the far side of the table target the task area and look back toward the robot. Each layout uses the same registered cameras across D+/D−/C+/C−/I+/I− and all registered checkpoints. Robot, wrist camera, objects, supports, scoring and the 18 prompts are unchanged.
...
Offline projection checks cover 174 exterior views across the 87 prior measured geometries, including every tabletop object (including banana) and support, both goal destinations and a 0.12 m lift envelope, with a minimum boundary margin of 36.21 native pixels. These projections are not visibility proof during arbitrary arm motion. On the six native layouts, projected cube width after proportional 320×180 resizing is 20.8 px in LAT, 14.5–18.2 px in HEIGHT and 13.8–17.9 px in DIST; these are framing measurements, not model performance. Offline destination materialization passed for all 87 revised layouts and the current 1,566 N3/E3/F3 cell bindings; see the current delivery index. Offline image preprocessing also passed on six native inputs and one synthetic input without model requests. Full live model transforms, generated-future time maps and semantic-label validation remain separate gates. See the [camera alignment record](../CAMERA_ALIGNMENT.md) and [current delivery index](../../handoff/delivery.json).
...
A generated frame is scored only when its physical target time, source camera and preprocessing are documented, and the time lies inside the action prefix executed before replanning. N3 executes up to 32 actions per request; E3/F3 prefixes must be fixed from their qualified official configurations. Account for context frames, cropping and packed views. Presentation FPS is not a time map, and the availability of a camera does not establish which image the model consumed. Record actual policy inputs and corresponding continuous simulator views.
...
Retain cube and anchor localization separately, including both DIST anchors. Two blinded raters locate centers; discrepancies above 0.02 image diagonals require adjudication. Average selected requests within an episode, then paired conditions within a layout. The maximum initial confirmation workload is 2,592 selected requests and 15,552 frame judgments before adjudication. Continuous accuracy is conditional on valid localization, with coverage reported for every condition.
...
2. **Qualify all three model interfaces and recording.** Verify resets, actual model inputs, decoded futures, action prefixes and physical time maps for N3, E3 and F3. The pilot validates measurement; development fixes H and analysis choices before confirmation.
...
4. **Complete the analysis export.** Produce all nine model-family coverage rows, language-effect tables, physical separation, prediction/persistence skill, disagreement, and side/stage diagnostics. The physical-separation correction is implemented; full coverage accounting, prediction metrics and the two-endpoint equivalence calculation still need completed outputs.
...
The study remains limited to D/C/I × LAT/HEIGHT/DIST × N3/E3/F3 in the clean setting. The planned learned-policy run starts only after the separate launch decision. The eventual paper must report unavailable measurements explicitly, including the possibility that generated futures add little diagnostic value beyond execution scores.
