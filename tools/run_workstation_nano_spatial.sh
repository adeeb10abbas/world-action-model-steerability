#!/usr/bin/env bash
# Nine native RoboLab spatial tasks with the same Nano policy on the workstation.
set -euo pipefail
run_root=${1:-/home/ali/wam-nano-stock-20260926}
attempt=${2:-spatial-suite-001}
...
# Save the prospective selection and runner identity before loading the model.
cp "$run_root/code/SOURCE_COMMIT" "$attempt_root/SOURCE_COMMIT"
PYTHONPATH="$run_root/code/tools/workstation_nano" /home/ali/cosmos-framework/.venv/bin/python - "$attempt_root" <<'PYPLAN'
import pathlib, sys
from common import dump
from stock_tasks import plan
root = pathlib.Path(sys.argv[1])
dump(root / "prospective-plan.json", plan())
PYPLAN
...
/home/ali/cosmos-framework/.venv/bin/torchrun --standalone --nproc_per_node=2 \
  ...
  --offload-fsdp \
  --max-requests 133 --wall-seconds 14400 --request-timeout 900 \
  > "$attempt_root/server.log" 2>&1 &
server_pid=$!
printf '%s\n' "$server_pid" > "$attempt_root/server.pid"
trap 'runner_status=$?; printf "%s\n" "$runner_status" > "$attempt_root/exit-code"; kill "$server_pid" 2>/dev/null || true; wait "$server_pid" 2>/dev/null || true' EXIT
...
while True:
    ...
    time.sleep(2)
print('Nano server is ready; starting the nine stock spatial tasks.', flush=True)
PY
...
env -u DISPLAY -u WAYLAND_DISPLAY timeout --signal=TERM --kill-after=30s 14400 \
  /home/ali/sgw-scene-design-20260923/venv/bin/python \
  "$run_root/code/tools/workstation_nano/run_stock_tasks.py" \
...
date -u
printf 'Nine-task runner finished. Results: %s\n' "$attempt_root/episodes"
