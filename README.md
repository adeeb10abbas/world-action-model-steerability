...
**Roster revision 1.2:** DreamZero is retired from the executable queue. All three pinned runtimes completed six genuine fixed-input and two live requests each on A100: 24 requests and 192 receiver-confirmed live actions, with zero study episodes. Nano's original cleanup failure remains preserved alongside a separate external lifecycle witness; Edge and FLUX completed cleanly. Physical forecast alignment remains unavailable. The durable controller is deployed; an authorized prospective amendment moves all three policy cohorts to A40 because live user-owned queues occupy the A100 allocations. The first genuine study request per model will establish A40 compatibility; prior A100 evidence is not relabeled. See the [current cluster continuation](handoff/cluster-execution-20260924/README.md) for exact evidence and scope.
