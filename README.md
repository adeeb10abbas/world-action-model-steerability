...
**Roster revision 1.2:** DreamZero is retired from the executable queue. All three pinned runtimes have now completed six genuine fixed-input requests each; FLUX same-request capture preserved actions exactly in its on/off comparison. Closed-loop and physical forecast qualification remain pending, with zero study episodes. The user has separately authorized cluster execution subject to those gates. See the [current cluster continuation](handoff/cluster-execution-20260924/README.md) for deployed source, persistent bindings, preserved failures and reproducible startup repairs.
